...
- `/auth/signup`, `/login`, `/verify-email`, `/forgot-password`, `/reset-password` → `(auth)/` stack. Supabase Auth API identical. Google OAuth via browser (`signInWithOAuth` + `skipBrowserRedirect` + `expo-web-browser.openAuthSessionAsync`, redirect `sensorium://auth/callback` in `src/lib/google-auth.ts`); deep-link `exp://` / EAS URL for verify + reset via `expo-linking` + `expo-web-browser` instead of web redirects. Test against staging Inbucket/local stack first. Supabase dashboard must allowlist the `sensorium://auth/callback` (plus dev `exp://`) redirect and have the Google provider enabled.
