...
- **Seen-vs-clear center (0116, 0133)**: `get_my_notifications` returns recent read and unread stored rows, so a single read greys the card instead of dropping it; only "Mark all read" empties the page (`mark_all_read` deletes the caller's stored rows and advances chat watermarks). Since 0156 the center holds stored rows only; opening a room advances the cluster watermark that clears that cluster's card badge
(`get_unread_chat_counts`). The header badge counts stored unread only, so plain chat never badges the
Notifications tab.
...
- **Plain-chat push (0153)**: the message-to-`push_outbox` fan-out trigger so plain chat writes produce pushes while plain chat stays out of the stored inbox (mentions and other events only).
- **Read-receipt batch cap + outbox retry indexes (0154)**: `mark_cluster_read` / `mark_all_read` freeze receipts in bounded 2000-row batches (same rows, same instant, same watermark), with room screens throttled to one mark per 5s plus a hide/background/leave flush (web `RoomView`, mobile `room.tsx`; leaving while pinned fires a pending trailing mark so the card badge never goes stale), partial retry indexes cover the outbox claim predicates (`queued` + retryable `failed`).
