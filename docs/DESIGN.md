...
Dark mode is **"Ink & Parchment"**, a warm inversion inspired by the Brutal Luxury treatment of emmanuelmathew.dev. The warm cream surfaces of the light theme become a neutral warm charcoal (`#181818`), text becomes warm cream (`#fcf9f2`), and interactive elements use blue (`#2F6BEE`, white text 4.71:1, AA) that holds contrast on dark while staying comfortable for prolonged nighttime use. Dark borders use `outline-variant` (`#404040`) so cards and inputs stay visibly bounded on `#181818`. Surfaces stay **neutral** (not brown-tinted) so the dark theme reads clean, editorial, and calm, never cold, never muddy.
