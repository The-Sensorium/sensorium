...
type PostRealtime = Database['public']['Tables']['posts']['Row']
type PostCommentRealtime = Database['public']['Tables']['post_comments']['Row']
type PostLikeRealtime = Database['public']['Tables']['post_likes']['Row']
type CommentLikeRealtime = Database['public']['Tables']['comment_likes']['Row']
 ...
 * its call belongs to (participants carry no cluster id).
 */
 ...
 * here: the first mounter owns the channel, later ones just bump the ref.
 */
interface ClusterChannelEntry {
  refs: number
  teardown: () => void
}
...
export function useClusterChannel(clusterId: string | null) {  const queryClient = useQueryClient()
  ...
  useEffect(() => {
    if (!clusterId) return
    const existing = clusterChannelEntries.get(clusterId)
    if (existing) {
      existing.refs += 1
      return () => {
        existing.refs -= 1
        if (existing.refs <= 0) {
          clusterChannelEntries.delete(clusterId)
          existing.teardown()
        }
      }
    }
    const entry: ClusterChannelEntry = { refs: 1, teardown: () => {} }
    clusterChannelEntries.set(clusterId, entry)
    const supabase = requireSupabase()
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          queryClient.setQueryData<PostRealtime[]>(postsKey, (cur) => {
